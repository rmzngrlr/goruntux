...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // SILINDI (v3.65): app.py'de KARSILIGI OLMAYAN 6 uca POST atan yetim isleyiciler —
  // updateServerStatus (/api/extension/update_status), submitServerResult ve ikinci bir
  // gonderim blogu (/api/extension/submit_result), submitLocalResult
  // (/api/extension/submit_local_result), updateTweetViews (/api/tweets/update_views),
  // saveInteractionsToServer (/api/tweets/save_interactions), closeActiveTab.
  // Hepsi onceki X otomasyon programinin sunucu API'sine konusuyordu; bu depoda o rotalar
  // YOK, yani istekler 404 alip .catch ile sessizce yutuluyordu. Ucu ayrica
  // credentials:'include' ile gonderiyordu — yani ucuncu bir origin'e kullanici cerezi.
  // Cagiranlari widget.js'in olu kazima motorundaydi, o da bu commit'te silindi.
